...
func Get_playlistsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["persona_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("persona_id=%v", val))
		}
...
func CreateGet_playlistsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_playlists",
		mcp.WithDescription("Returns playlists optionally filtered by {start} and/or {end} datetimes"),
		mcp.WithString("start", mcp.Description("The datetime starting from items must be returned. Maximum 1 hour in future.\n")),
		mcp.WithString("end", mcp.Description("The ending datetime. Maximum 1 hour in future.\n")),
		mcp.WithNumber("show_id", mcp.Description("Filter by show")),
		mcp.WithNumber("persona_id", mcp.Description("Filter by persona")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_playlistsHandler(cfg),
	}
}
