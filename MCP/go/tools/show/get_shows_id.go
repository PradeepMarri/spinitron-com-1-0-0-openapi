...
func Get_shows_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/shows/%s%s", cfg.BaseURL, id, queryString)
...
func CreateGet_shows_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_shows_id",
		mcp.WithDescription("Get a Show by id"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_shows_idHandler(cfg),
	}
}
